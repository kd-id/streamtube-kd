...
export const DEFAULT_AI_PROVIDERS = [
  { id: 'gemini', name: 'Google Gemini', short: 'Gemini', type: 'gemini', system: true, capabilities: ['text'], baseUrl: '', endpoints: { models: '', chat: '' } },
  { id: 'openai', name: 'OpenAI', short: 'OpenAI', type: 'openai-compatible', system: true, capabilities: ['text'], baseUrl: 'https://api.openai.com/v1', endpoints: { models: '/models', chat: '/chat/completions' } },
  { id: 'anthropic', name: 'Anthropic', short: 'Claude', type: 'anthropic', system: true, capabilities: ['text'], baseUrl: 'https://api.anthropic.com/v1', endpoints: { messages: '/messages' } },
  { id: 'grok', name: 'xAI Grok', short: 'Grok', type: 'openai-compatible', system: true, capabilities: ['text'], baseUrl: 'https://api.x.ai/v1', endpoints: { models: '/models', chat: '/chat/completions' } },
  { id: 'groq', name: 'Groq', short: 'Groq', type: 'openai-compatible', system: true, capabilities: ['text'], baseUrl: 'https://api.groq.com/openai/v1', endpoints: { models: '/models', chat: '/chat/completions' } },
  { id: 'openrouter', name: 'OpenRouter', short: 'Router', type: 'openai-compatible', system: true, capabilities: ['text'], baseUrl: 'https://openrouter.ai/api/v1', endpoints: { models: '/models', chat: '/chat/completions' } },
  { id: 'devin', name: 'Devin.ai', short: 'Devin', type: 'devin', system: true, capabilities: ['agent'], baseUrl: 'https://api.devin.ai/v1', endpoints: { sessions: '/sessions' } },
  { id: 'leonardo', name: 'Leonardo.Ai', short: 'Leonardo', type: 'leonardo', system: true, capabilities: ['image', 'video'], baseUrl: 'https://cloud.leonardo.ai/api/rest', endpoints: { image: '/v2/generations', imageLegacy: '/v1/generations', video: '/v2/generations', videoImage: '/v1/generations-image-to-video', status: '/v1/generations/{{id}}' } },
];
...
const DEFAULT_MODEL_MAP = {
  ...
  devin: ['devin-session'],
  leonardo: ['gpt-image-2', 'gpt-image-1.5', 'gemini-2.5-flash-image', 'gemini-image-2', 'nano-banana-2', 'seedream-4.5', 'flux-pro-2.0', 'hailuo-2_3', 'kling-3.0', 'kling-video-o-3', 'ltxv-2.3-pro', 'seedance-1.0-pro', 'VEO3', 'VEO3_1', 'KLING2_5', 'KLING2_1'],
};
...
function cloneProviders(providers) {
  return providers.map(p => ({ ...p, capabilities: [...(p.capabilities || [])], endpoints: { ...(p.endpoints || {}) } }));
}
...
function makeProviderId(name, existingIds) {
  ...
  let id = base; let idx = 2;
  while (existingIds.has(id)) { id = `${base}-${idx}`; idx += 1; }
  return id;
}
...
function defaultEndpointsForType(type) {
  ...
  if (type === 'leonardo') return { image: '/v2/generations', imageLegacy: '/v1/generations', video: '/v2/generations', videoImage: '/v1/generations-image-to-video', status: '/v1/generations/{{id}}' };
...
function mergeProviders(savedProviders, savedConfig = {}) {
  ...
  const customProviders = saved
    .filter(p => p?.id && !DEFAULT_IDS.has(p.id) && p.id !== 'custom')
  ...
  if ((savedConfig?.apiKeys?.custom || savedConfig?.baseUrls?.custom) && !customProviders.some(p => p.id === 'custom-provider')) {
    customProviders.push(normalizeProvider({ id: 'custom-provider', name: 'Custom Provider', short: 'Custom', type: 'openai-compatible', baseUrl: savedConfig?.baseUrls?.custom || '' }));
  }
...
async function parseOpenAIResponse(res) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    try { return JSON.parse(lines[i].slice(6)); } catch {}
  }
...
export function AIProvider({ children }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!token) { dispatch({ type: 'UPDATE_CONFIG', payload: { aiReady: true } }); return; }
      try {
        ...
        if (data.success && data.data) {
          ...
          if (initialApiKeys.custom && !initialApiKeys['custom-provider']) { initialApiKeys['custom-provider'] = initialApiKeys.custom; delete initialApiKeys.custom; }
          if (initialBaseUrls.custom && !initialBaseUrls['custom-provider']) { initialBaseUrls['custom-provider'] = initialBaseUrls.custom; delete initialBaseUrls.custom; }
          dispatch({
            type: 'UPDATE_CONFIG',
            payload: {
              ...initialState, ...saved, provider, apiKey: '', baseUrl: '',
              apiKeys: initialApiKeys, baseUrls: initialBaseUrls,
          ...
          dispatch({ type: 'UPDATE_CONFIG', payload: { aiReady: true } });
        }
      } catch { dispatch({ type: 'UPDATE_CONFIG', payload: { aiReady: true } }); }
    };
    init();
  }, []);
  ...
  const getEffectiveKey = useCallback((prov) => stateRef.current.apiKeys?.[prov] || '', []);
  const getEffectiveBase = useCallback((prov) => {
    const cur = stateRef.current;
    return cur.baseUrls?.[prov] || getProviderFromState(cur, prov)?.baseUrl || '';
  }, []);
  ...
  const updateConfig = useCallback((updates) => {
    ...
    const nextState = { ...cur, ...updates, apiKey: '', baseUrl: '', apiKeys: newApiKeys, baseUrls: newBaseUrls, modelName: updates.modelName !== undefined ? sanitizeModel(updates.modelName) : cur.modelName };
    dispatch({ type: 'UPDATE_CONFIG', payload: nextState });
    saveConfig(nextState);
  }, [saveConfig]);

  const updateProviderDetails = useCallback((provId, details) => {
    const cur = stateRef.current;
    const provider = getProviderFromState(cur, provId);
    if (!provider) return { success: false, error: 'Provider not found' };
    const endpoints = { ...(provider.endpoints || {}), ...(details.endpoints || {}) };
    const providers = cur.providers.map(p => p.id === provId ? normalizeProvider({ ...p, name: details.name ?? p.name, short: details.short ?? p.short, type: details.type ?? p.type, baseUrl: details.baseUrl ?? p.baseUrl, endpoints }) : p);
  ...
  const addProvider = useCallback((details) => {
    const cur = stateRef.current;
    const existingIds = new Set((cur.providers || []).map(p => p.id));
    const id = makeProviderId(details.name || 'Provider', existingIds);
    const provider = normalizeProvider({ id, name: details.name || 'New Provider', short: details.short || details.name || 'Provider', type: details.type || 'openai-compatible', system: false, baseUrl: details.baseUrl || '', endpoints: { ...defaultEndpointsForType(details.type || 'openai-compatible'), ...(details.endpoints || {}) } });
    ...
    const nextState = { ...cur, providers, provider: nextProvider, modelName: nextModel, providerModels: { ...cur.providerModels, [id]: details.modelName ? [details.modelName] : [] } };
    dispatch({ type: 'UPDATE_CONFIG', payload: nextState });
    saveConfig(nextState);
    return provider;
  }, [saveConfig]);

  const deleteProvider = useCallback((provId) => {
    const cur = stateRef.current;
    const provider = getProviderFromState(cur, provId);
    if (!provider) return { success: false, error: 'Provider not found' };
    ...
    const apiKeys = { ...cur.apiKeys }; const baseUrls = { ...cur.baseUrls };
    const providerEndpoints = { ...cur.providerEndpoints }; const providerModels = { ...cur.providerModels };
    delete apiKeys[provId]; delete baseUrls[provId]; delete providerEndpoints[provId]; delete providerModels[provId];
    const fallback = providers.find(p => providerHas(p, 'text'))?.id || 'gemini';
    const nextState = { ...cur, providers, apiKeys, baseUrls, providerEndpoints, providerModels, provider: cur.provider === provId ? fallback : cur.provider, modelName: cur.provider === provId ? (providerModels[fallback]?.[0] || DEFAULT_MODEL_MAP[fallback]?.[0] || '') : cur.modelName };
    dispatch({ type: 'UPDATE_CONFIG', payload: nextState });
    saveConfig(nextState);
    return { success: true };
  }, [saveConfig]);

  const saveProviderModels = useCallback((provId, models) => {
    const cur = stateRef.current;
  ...
  const deleteApiKey = useCallback((provId, singleKey) => {
  ...
  const addApiKeys = useCallback((provId, newKeysRaw) => {
    const prov = provId || stateRef.current.provider;
    const newApiKeys = { ...stateRef.current.apiKeys };
  ...
  const fetchAvailableModels = useCallback(async (prov, rawKey, base) => {
    ...
    if (provider.type === 'gemini') {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        ...
        } catch (e) { if (i === keys.length - 1) throw e; }
      }
    }
  ...
  const testConnection = useCallback(async (prov, rawKey, base, model, prompt = 'Say OK') => {
    ...
    if (provider.type === 'gemini') {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        ...
        } catch (e) { results.push(`${keyLabel}: FAILED ${e.message}`); }
      }
  ...
  const callProviderAPI = useCallback(async (provider, model, key, base, promptText, maxTokens = MAX_TOKENS.generate) => {
  ...
  const extractText = useCallback(({ data, provider: prov }) => {
  ...
  const isRetryable = useCallback(({ data, status }) => {
    const code = data?.error?.code || status;
    return code === 429 || code === 503 || code === 529;
  }, []);

  const tryProvider = useCallback(async (provId, promptText, maxTokens) => {
    const cur = stateRef.current;
    ...
    const keys = provDef.type === 'gemini' ? Array.from(new Set(rawKey.split(/[\n,]+/).map(k => k.trim()).filter(Boolean))) : [rawKey.trim()];
    ...
    if (models.length === 0) models.push('gpt-4o', 'gpt-4o-mini', 'gpt-3.5-turbo');
    for (let mIdx = 0; mIdx < models.length; mIdx += 1) {
      ...
      for (let kIdx = 0; kIdx < keys.length; kIdx += 1) {
        ...
        try {
          ...
          break;
        } catch (err) { console.warn(`[AI] ${provId}/${model} failed: ${err.message}`); }
      }
    }
    return { success: false, reason: 'all_models_and_keys_failed' };
  }, [getEffectiveKey, getEffectiveBase, callProviderAPI, extractText, isRetryable]);

  const generateText = useCallback(async (promptText, maxTokens = MAX_TOKENS.generate) => {
    checkRateLimit();
    const cur = stateRef.current;
    ...
    if (activeProvider?.type === 'devin') {
      ...
      const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` }, body: JSON.stringify({ prompt: promptText }) });
      const data = await res.json();
    ...
    const cascade = Array.from(new Set([...(activeId ? [activeId] : []), ...DEFAULT_TEXT_CASCADE, ...allTextProviders]));
  ...
  }, [getEffectiveKey, getEffectiveBase, getEffectiveEndpoint, tryProvider]);

  const generateAllMeta = useCallback(async ({ context, theme = '', category = '' } = {}) => {
    ...
    const tags = tagsRaw.replace(/[\n\r]+/g, ', ').replace(/^\s*[-*]\s*/gm, '').replace(/^\s*\d+[\.)]\s*/gm, '').split(',').map(t => t.trim().replace(/^["']|["']$/g, '')).filter(t => t.length > 0 && t.length < 60).slice(0, 20);
  ...
  return (
    <AIContext.Provider value={{
      config: state, updateConfig, addProvider, deleteProvider, updateProviderDetails,
      deleteApiKey, addApiKeys, generateText, generateAllMeta, fetchAvailableModels,
      testConnection, getEffectiveKey, getEffectiveBase, getEffectiveEndpoint,
      saveProviderModels, getRemainingRequests,
    }}>
      {children}
    </AIContext.Provider>
  );
...
export function useAIStore() {
  ...
  if (!ctx) throw new Error('useAIStore must be used within AIProvider');
  return ctx;
}
